...
def compute_moments_shift(
    observed: pd.Series,
    imputed: pd.Series,
    names: Tuple[str, str] = ("observed", "imputed"),
) -> Dict[str, float]:
    """Summary of overall shift of mean and std. dev. of predictions for a imputation method."""
    name_obs, name_model = names
    data = {
        name: {"mean": series.mean(), "std": series.std()}
        for series, name in zip([observed, imputed], names)
    }
    observed, imputed = data[name_obs], data[name_model]
    data[name_model]["mean shift (in std)"] = (
...
def stats_by_level(
    series: pd.Series, index_level: int = 0, min_count: int = 5
) -> pd.Series:
