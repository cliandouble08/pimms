...
# %% tags=["parameters"]
FN_INTENSITIES: str = (
    "data/dev_datasets/HeLa_6070/protein_groups_wide_N50.csv"  # Sample (rows), features (columns)
)
index_col: Union[str, int] = (
    0  # Can be either a string or position (default 0 for first column), or a list of these.
)
column_names: List[str] = [
    "Gene Names"
]  # Manuelly set column names (of Index object in columns)
fn_rawfile_metadata: str = (
    "data/dev_datasets/HeLa_6070/files_selected_metadata_N50.csv"  # metadata for samples (rows)
)
feat_prevalence: Union[int, float] = (
    0.25  # Minimum number or fraction of feature prevalence across samples to be kept
)
sample_completeness: Union[int, float] = (
    0.5  # Minimum number or fraction of total requested features per Sample
)
...
logarithm: str = (
    "log2"  # Log transformation of initial data (select one of the existing in numpy)
)
...
use_every_nth_xtick: int = (
    1  # use every nth xtick in plots (default 1, i.e. every xtick is kept)
)
...
frac_mnar: float = (
    0.0  # fraction of missing not at random data, rest: missing completely at random
)
...
if not 0.0 <= args.frac_mnar <= 1.0:
    raise ValueError(
        "Invalid MNAR float value (should be betw. 0 and 1):" f" {args.frac_mnar}"
    )
...
if args.logarithm:
    ...
    df = log_fct(
        df
    )  # ! potentially add check to increase value by 1 if 0 is present (should be part of preprocessing)
df

# %% tags=["hide-input"]
ax = df.notna().sum(axis=0).to_frame(df.columns.name).plot.box()
...
# %% tags=["hide-input"]
if args.meta_date_col:
    df_meta[args.meta_date_col] = pd.to_datetime(df_meta[args.meta_date_col])
else:
...
try:
    display(
        meta_stats.loc[
            :, (meta_stats.loc["unique"] > 1) | (meta_stats.loc["std"] > 0.1)
        ]
    )
except KeyError:
...
if args.select_N is not None:
    ...
    df = df.loc[df_meta.index].dropna(how="all", axis=1)
...
if isinstance(args.feat_prevalence, float):
    ...
    args.overwrite_entry("feat_prevalence", int(N_samples * args.feat_prevalence))
...
logger.info(f"Feature has to be present in at least {args.feat_prevalence} of samples")
# select features
...
if isinstance(args.sample_completeness, float):
    ...
    args.overwrite_entry(
        "sample_completeness", int(df.shape[1] * args.sample_completeness)
    )
...
logger.info(msg)
df = df.loc[mask]
df = df.dropna(axis=1, how="all")  # drop now missing features
...
_ = ax.set_xticklabels(_new_labels, rotation=45, horizontalalignment="right")
...
# %% tags=["hide-input"]
ax, _data_feat_median_over_prop_missing = (
    pimmslearn.plotting.data.plot_feat_median_over_prop_missing(
...
if args.meta_date_col != "PlaceholderTime":
    ...
    analyzers.plot_date_map(
        df=pcs[pcs_name],
        ax=ax,
        dates=pcs[args.meta_date_col],
        title=f"by {args.meta_date_col}",
    )
...
# %% tags=["hide-input"]
ax = df_w_date.plot.box(
    rot=80,
    figsize=(7, 3),
    fontsize=7,
    showfliers=False,
    showcaps=False,
    boxprops=dict(linewidth=0.4, color="darkblue"),
    flierprops=dict(markersize=0.4, color="lightblue"),
)
...
if not args.meta_date_col == "PlaceholderTime":
    ...
    median_sample_intensity = df.median(axis=1).to_frame("median intensity")
    median_sample_intensity = median_sample_intensity.join(dates)

    ax = median_sample_intensity.plot.scatter(
        x=dates.name,
        y="median intensity",
        rot=90,
        #   fontsize=6,
        figsize=(8, 2),
        s=5,
        xticks=pimmslearn.plotting.select_dates(median_sample_intensity[dates.name]),
    )
    fig = ax.get_figure()
...
logger.info(
    f"{splits.train_X.shape = } - {splits.val_y.shape = } - {splits.test_y.shape = }"
)
...
ax = axes[0]
plot_histogram_intensities = partial(
    pimmslearn.plotting.data.plot_histogram_intensities, min_max=min_max, alpha=0.8
)
plot_histogram_intensities(df_long.squeeze(), ax=ax, label="observed")
plot_histogram_intensities(thresholds, ax=ax, label="thresholds")
if args.use_every_nth_xtick > 1:
    ax.set_xticks(ax.get_xticks()[::2])
ax.legend()
ax = axes[1]
plot_histogram_intensities(
    fake_na_mnar.squeeze(), ax=ax, label=f"MNAR ({N_MNAR:,d})", color="C2"
)
plot_histogram_intensities(
    fake_na_mcar.squeeze(), ax=ax, color="C3", label=f"MCAR ({N_MCAR:,d})"
)
if args.use_every_nth_xtick > 1:
    ax.set_xticks(ax.get_xticks()[::2])
ax.legend()
...
if 0.0 < args.prop_sample_w_sim < 1.0:
    ...
    if args.meta_cat_col and df_meta is not None:
        to_stratify = df_meta[args.meta_cat_col].fillna(
            -1
        )  # ! fillna with -1 as separate category (sofisticate check)
    train_idx, val_test_idx = train_test_split(
        splits.train_X.index.levels[0],
    ...
    logger.info(
        f"Sample in Train: {len(train_idx):,d} - Validation: {len(val_idx):,d} - Test: {len(test_idx):,d}"
    )
    # reassign some simulated missing values to training data:
    splits.train_X = pd.concat(
        [splits.train_X, splits.val_y.loc[train_idx], splits.test_y.loc[train_idx]]
    )
    splits.val_y = splits.val_y.loc[val_idx]
    splits.test_y = splits.test_y.loc[test_idx]
    logger.info(
        f"New shapes: {splits.train_X.shape = } - {splits.val_y.shape = } - {splits.test_y.shape = }"
    )
...
if mask_min_4_measurments.any():
    ...
    logger.warning(
        f"Features with less than 4 measurments in training data: {idx.to_list()}"
    )
    ...
    if mask_min_4_measurments.any():
        idx = mask_min_4_measurments.loc[mask_min_4_measurments].index
        raise ValueError(
            "Some features still have less than 4 measurments in training data"
...
_ = splits.val_y.plot.hist(bins=bins, xticks=list(bins), ax=ax, color="C2", legend=True)
ax.legend(_legend[:-1])
if args.use_every_nth_xtick > 1:
    ax.set_xticks(ax.get_xticks()[::2])
...
# %% tags=["hide-input"]
ax = splits_df.drop("train", axis=1).plot.hist(
    bins=bins,
    xticks=list(bins),
    color=["C1", "C2"],
    legend=False,
...
# %% tags=["hide-input"]
medians = (splits.train_X.median().astype(int)).to_frame("median_floor")
...
s_axes = pd.DataFrame(
    ...
    by="medians", boxprops=dict(linewidth=s), flierprops=dict(markersize=s), ax=ax
)
for ax in s_axes:
...
fname = (
    args.out_figures / f"0_{group}_intensity_median_vs_prop_missing_boxplot_val_train"
)
figures[fname.stem] = fname
pimmslearn.savefig(ax.get_figure(), fname)
