...
def test_get_dls():
    ...
    N_valid = int(N * 0.3)
    X_valid = create_random_df(N_valid, M, prop_na=0.1, start_idx=len(X_train))
    ...
    transforms = VaepPipeline(
        df_train=X_train, encode=dae_default_pipeline, decode=["normalize"]
    )
    BS = 4
