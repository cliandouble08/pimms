...
def example_data():
    ...
    data = [
        [25.47317633, 27.23206642, 26.43510602, 28.40661375, 27.6536975],
    ...
    data = pd.DataFrame(
        data,
        index=(f"feat_{i}" for i in [0, 0, 1, 1, 1, 2, 3, 4, 5, 6]),
...
def test_get_absolute_error(example_data):
    ...
    actual = (
        calc_errors.get_absolute_error(example_data.drop("freq_feat", axis=1))
        .reset_index()
        .to_dict()
    )
...
def test_calc_errors_per_feat(example_data):
    ...
    actual = (
        calc_errors.calc_errors_per_feat(
            pred=example_data.drop("freq_feat", axis=1),
            freq_feat=example_data["freq_feat"],
        )
        .reset_index()
        .to_dict()
    )
...
def test_calc_errors_per_bin(example_data):
    ...
    actual = (
        calc_errors.calc_errors_per_bin(example_data.drop("freq_feat", axis=1))
        .sort_values(["n_obs", "bin", "model_1"])
        .reset_index()
        .to_dict()
    )
    assert actual == expected
